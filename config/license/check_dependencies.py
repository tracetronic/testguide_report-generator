...
class ComponentValidator:
    ...
    def __name(self):
        if self.allow["moduleName"] != self.sbom["name"]:
            self.append_error("name", self.allow["moduleName"], self.sbom["name"])
    ...
    def __licenses(self):
        ...
        if not (any([allow_license == lic for lic in prepared_license_list])):
...
def compare_license_files(allowlist_path, sbom_path, allowschema_path):
    ...
    for component in sbom_json["components"]:
        ...
        for allowed_component in allow_json["allowedLicenses"]:
            ...
            name = allowed_component["moduleName"] == component["name"]
            if name:
...
def main(argv):
    allow_filepath = ''
    sbom_filepath = ''
    schema_filepath = "config/license/allowlist_schema.json"
    try:
...
if __name__ == "__main__":
    # generate sbom_path
    os.system("cyclonedx-py -pip --format json")
